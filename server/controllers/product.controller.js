...
const getCategoryAndChildrenIds = async (categoryId) => {
  ...
  const collect = (id) => {
    ...
    categories.forEach((cat) => {
      if (cat.parent && cat.parent.toString() === id.toString()) {
        collect(cat._id);
      }
    });
  };
...
const updateProduct = async (req, res) => {
  try {
    ...
    const safeVariants = variants.map((variant) => ({
      ...
      sku: variant.sku?.trim()
        ? variant.sku
        : generateVariantSku(req.body.title, variant.attributes || {}),
    }));
    ...
    const duplicateInRequest = new Set(skus).size !== skus.length;
    ...
    if (productFiles.length) {
      ...
      uploadedProductImages = await Promise.all(uploadTasks);
    }

    const finalImages = [...existingImages, ...uploadedProductImages];
    ...
    if (variantFiles.length) {
      for (const file of variantFiles) {
        ...
        uploadedVariantImages.push(result.secure_url);
      }
    }
    ...
    const finalVariants = safeVariants.map((variant, index) => {
      ...
      variantImageIndexes.forEach((variantIndex, uploadIndex) => {
        if (Number(variantIndex) === index) {
          const uploadedImage = uploadedVariantImages[uploadIndex];
      ...
      return {
        ...
        images: [...(variant.existingImages || []), ...newImages],
    ...
    let updatedSellers = product.sellers.map((sellerItem) => {
      ...
      // SELLER UPDATE
      if (sellerItem.seller.toString() === sellerId) {
        return {
          ...sellerItem.toObject(),
